...
*应用TensorFlow和 Keras 研讨会*首先向您展示神经网络是如何工作的。在你理解了基础知识之后，你将通过改变它们的超参数来训练一些网络。为了提高您的技能，您将学习如何选择最合适的模型来解决手头的问题。在处理高级概念时，您将发现如何通过汇集构建基本深度学习系统所需的所有基本元素来组装深度学习系统——数据、模型和预测。最后，您将探索评估模型性能的方法，并使用模型评估和超参数优化等技术对其进行改进。
