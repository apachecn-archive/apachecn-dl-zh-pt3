...
基于语言模型的嵌入代表了词嵌入发展的下一步。语言模型是单词序列的概率分布。一旦我们有了一个模型，我们就可以让它预测给定特定单词序列的最有可能的下一个单词。类似于传统的静态和动态词嵌入，它们被训练来预测来自语料库的给定部分句子的下一个单词(或者前一个单词，如果语言模型是双向的)。训练不涉及主动标注，因为它利用了大量文本的自然语法结构，所以在某种意义上，这是一个自我监督的学习过程。
