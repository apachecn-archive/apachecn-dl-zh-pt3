...
DBNs 可以被认为是简单的无监督网络的组合，例如**受限玻尔兹曼机器** ( **RBMs** )或自动编码器；在这些子网中，每个子网的隐藏层充当下一个子网的可见层。RBM 是一种无方向的生成模型，具有一个输入层(可见)和一个隐藏层，层与层之间有连接，但层与层之间没有连接。这种拓扑导致了快速、逐层、无监督的训练过程。从最低的一对层(最低的可见层是训练集)开始，将对比散度应用于每个子网。
...
![](Images/70002aa2-1334-419c-818a-23a3a0021660.png)<title>Unknown</title>  <link href="../stylesheet.css" rel="stylesheet" type="text/css"> <link href="../page_styles.css" rel="stylesheet" type="text/css">
...
*   `graph` : `tf.Graph, optional (default = None)`，张量流图形对象
