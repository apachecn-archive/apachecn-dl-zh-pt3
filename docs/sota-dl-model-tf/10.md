...
正如我们所知，一个*生成器*被用来从问题域中生成新的看似合理的例子。它将随机分布作为输入(通常是高斯分布)，并输出一些数据(通常是图像)。随机输入可以被认为是要生成的图像的潜在表示(或编码)。因此，发生器与可变自动编码器中的解码器具有相同的功能，并且可以以相同的方式用于生成新图像。然而，他们接受的训练非常不同。
