...
尽管处理后的内容和样式参考图像可由样式传输网络使用，但它期望每个图像作为单独的一批。所以每一批一定是形状*【batch _ size，image_height，image_width，3】*的 4D 张量。由于内容和样式参考图像目前是形状为*【image _ height，image_width，3】*的 3D 张量，我们添加了一个批量维度“1”来满足要求。网络还要求图像作为张量流张量。所以我们把它们转换成张量流张量。
...
该函数将 PIL 图像转换为 NumPy 数组。然后，它对数组进行缩放、调整大小，并将其转换为具有适当维数的张量流张量。
