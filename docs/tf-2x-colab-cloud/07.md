...
我们通过混洗训练数据、批处理、缩放、缓存和预取来构建输入管道。我们通过使用 lambda 函数映射来缩放图像。添加*缓存*方法可以提高 TFDS 的性能，因为数据只被读写一次，而不是在每个时期。添加*预取*方法是一个好主意，因为它增加了批处理过程的效率。也就是说，当我们的训练算法处理一批时，TensorFlow 正在并行处理数据集，以准备下一批。因此预取可以极大地提高训练成绩。
