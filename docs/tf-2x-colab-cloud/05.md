...
如清单 [5-4](#PC13) 所示，建立训练和测试数据的输入管道。
...
尽管建议将 TFDS 用于 TensorFlow 2.x 应用程序，但我们将向您展示如何使用 Keras 数据集，因为它在行业中很受欢迎。TensorFlow 2.x 的新颖性意味着它还没有 Keras 的行业渗透率。
...
为了理解张量流混淆矩阵，想象在矩阵的第一行上面从左到右是 0-9 类。这些代表预测。假设矩阵第一列的左边从上到下是 0-9 级。这些代表实际的标签。正确的分类是沿着对角线。误分类不在对角线上。
