...
对于 RNN，我们希望保存模型在每个时间步学到的东西。一种方法是用回调方法保存保存这些信息的检查点。**检查点**捕捉所有TensorFlow参数(或 tf)的精确值。Variable.objects)由模型使用。
...
对起始字符串进行矢量化，因为模型只识别数字。重塑TensorFlow消耗的矢量化起始字符串。也就是说，将 1 维添加到起始字符串，以便模型可以处理它。显示形状以验证一切正常。
