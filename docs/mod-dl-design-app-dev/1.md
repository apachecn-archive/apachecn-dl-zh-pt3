...
但是，如果损失函数更复杂，那么很难只用后端函数编写损失函数。在这种情况下，TensorFlow 的`.py_function()`允许将 Python 函数写成 TensorFlow 图，可以用作神经网络中的损失函数。例如，我们可以在我们的损失函数中编写`if/else`关系，否则需要用像`tf.cond()`这样的TensorFlow函数来编写，这允许条件语句，但很难使用并且不直观(清单 [1-9](#PC9) )。
...
### 来自图像文件的TensorFlow数据集

如果您正在使用影像数据集，它可能会被安排在一个目录中。不需要手动使用另一个库如`cv2`或`matplotlib`来将图像加载到存储器中，然后用现有方法将它们转换成TensorFlow数据集格式。TensorFlow 允许我们使用`.map`一步到位地自动读取图像并将其转换为 TensorFlow 数据集的元素，这允许我们对数据集的每个元素应用一个函数。
